...
The cumulative-bucket math generalizes the logic that already powers the existing file-at-62/file-at-70 comparison (used elsewhere in the calculator for a single person) to (a) a household sum and (b) a third "file at 67" case.
...
- **Calendar-axis end year.** Hard end = the later of the two people's age-100 year. For couples with a significant age gap this can push the axis past 50 years, which would crush the near-term, most decision-relevant years into a sliver if the initial view showed the whole span at once. Mitigation: the module's default/initial zoom shows roughly the next 10–15 years, with the full axis reachable by scrolling or zooming out, rather than rendering the entire span at full width by default.
