...
- **No "One Month at a Time" waiting-boost data yet.** The existing One Month at a Time feature (`frontend/src/components/OneMonthAtATime/`, `useBenefitCalculations.js`) shows the year-by-year lifetime income boost from delaying filing, which is a natural fit for this module's thesis. Deferred for now — flagged as on-deck, not scoped — because it uses its own calculation engine (back-derived PIA, its own inflation compounding, a hardcoded age-85 life-expectancy assumption) rather than `calculateProjection()`/`combineProjections()`, which this module standardizes on. Before this is picked up, decide whether to recompute the waiting-boost figures through the shared engine (recommended, for consistency with the rest of this module) or accept carrying two calculation philosophies side by side — and decide placement (folded into the existing tooltip vs. a separate annotation layer on the axis), which is also still undecided.
