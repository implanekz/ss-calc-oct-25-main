...
- **`survivorNote`** (present only when `prematureDeath` is true and `year >= deathYear`): describes that the `think` figure already reflects the survivor-benefit adjustment (the underlying number is already correct as of the "thread premature-death adjustment into household buckets" fix — this only adds a sentence explaining it). Wording is neutral about which spouse died — `combineProjections`' survivor adjustment keeps whichever benefit is larger from `deathYear` on and doesn't itself identify which person passed, and the "Death at ages:" input shows both ages rather than picking one — so the copy doesn't assert a specific person did: "This reflects survivor benefits — the household now receives the larger of the two benefits." (Revised 2026-08-24 from an earlier draft that named the primary specifically, which the math didn't support — caught in final review.)
